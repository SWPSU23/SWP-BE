...
const getPaySlipDetails = async (employee_id, month_year) => {
    try {
        ...
        const list_day_in_month_of_year = await pool
            .getData(
                queries.Calendar.getListDayOfMonthYear(month_year),
                []
            );
        const max_day_in_month = time.timeStampToDate(list_day_in_month_of_year[list_day_in_month_of_year.length - 1].date);
        const first_day_in_month = time.timeStampToDate(list_day_in_month_of_year[0].date);
        // get pay slip details
        const pay_slip_details = await pool
            .getData(
                queries.Salary.getPaySlipDetails(employee_id, first_day_in_month, max_day_in_month),
                []
            );
        // format data
        ...
        global.logger.error(`Model: getPaySlipDetails function has error ${error}`);
        throw error;
    }
}
...
const getPaySlip = async (employee_id, month_year) => {
    try {
        // get list day in month of year
        const list_salary_of_employee = await getPaySlipDetails(employee_id, month_year);
        const data = {
            employee_id: '',
            employee_name: '',
            employee_role: '',
